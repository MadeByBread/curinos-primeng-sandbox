...
A component with no PrimeNG 7 counterpart, authored in this repo. Either PrimeNG gained it in a later version (Avatar, Chip, Tag, Badge, OTP), never had it (Stepper's content panels, the app shell), or it is Curinos-specific (the logo). Chip, Tag and Badge share a Figma page with Chips; OTP is on the Inputs page. Chips is PrimeNG 7's multi-value input and is a Customized PrimeNG entry.

## PrimeNG 7 gap

Everything PrimeNG 7 ships that Curinos has no Figma page for. Components Figma already draws on another page — Editor on Textarea, FileUpload on Inputs, Slider as its own Inputs-page entry, ToggleButton and SplitButton on Button, AutoComplete on Selects — live on those catalogue entries, not here. Excludes behaviour-only directives, which have nothing to design, and the components PrimeNG 7 had already superseded.
...
The `/overview` page and the app's landing route. A theming reference: CSS-variable convention, the tokens components actually spend, and how those tokens land on stock PrimeNG. Written for someone meeting the system for the first time.
